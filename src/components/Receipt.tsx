        ...
        <div className="text-xs font-semibold text-slate-700 mb-4 flex flex-col gap-1.5">
          ...
          <div className="flex justify-between items-center mt-2 pt-2 border-t border-slate-200">
            ...
            <span className="font-bold text-slate-900">
              {(order.customerName ?? order.customer_name) === undefined ? '새로고침 필요' : (order.customerName || order.customer_name || '기록 없음')}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span>학번:</span>
            <span className="font-bold text-slate-900">
              {(order.customerStudentId ?? order.customer_student_id) === undefined ? '새로고침 필요' : (order.customerStudentId || order.customer_student_id || '기록 없음')}
            </span>
          </div>
        </div>
